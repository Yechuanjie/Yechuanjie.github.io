<template>
  <div class="meta-wrapper">
    <div class="meta-item original">
      <el-tag v-if="isOriginal" color="orangered" title="原创文章">
        <template #icon>
          <!-- <icon-trophy /> -->
        </template>
        原创
      </el-tag>
      <el-tag v-else color="arcoblue" title="转载文章">
        <template #icon>
          <!-- <icon-share-alt /> -->
        </template>
        转载
    ...
    <div class="meta-item tag">
      ...
      <!-- <span class="meta-content">
        <span v-for="(tag, index) in tags" :key="index">
          <a href="javascript:void(0);" @click="goToLink('/blog', 'tag', tag)" target="_self" :title="tag">{{ tag }}</a>
