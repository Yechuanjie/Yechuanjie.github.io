...
// 自动生成侧边栏
const autoSideBar = (dirPath: string) => {
  const files = fg.sync(`posts${dirPath}/**/[0-9]+-*.md`, { onlyFiles: true })
...
// 设置侧边栏
const setNavAndSide = () => {
  const files = fg.sync(`posts/**/[0-9]+-*.md`, { onlyFiles: true, ignore: ['posts/public/**/*'] })
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i]
    const fileEnd = file.replace(/^posts/gi, '')
    const fileSplit = fileEnd.split('/').filter((name) => name)

    if (fileSplit.length < 3 || fileSplit.length > 4) {
      console.log(
        `${colors.redBright(fileEnd)} 请按照 ${colors.greenBright('分类-[项目]-目录-文章')} 的层级方式组织文件`
      )
      continue
    }
    ...
    autoSideBar('/docs/posts/')

    // 设置导航下的擦边蓝
    // if (sideObjs[dirPath] === undefined) {
    //   sideObjs[dirPath] = autoSideBar('/docs/posts/')
    // }
